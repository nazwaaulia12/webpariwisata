...
@stop
@section('content')
<form action="{{route('daftar_paket.update', $daftarpaket)}}" method="post">
    @method('PUT')
    @csrf
    <div class="row">
        <div class="col-12">
...
@error('nama_paket') is-invalid @enderror" id="nama_paket" placeholder="Nama Paket" name="nama_paket" value="{{$daftarpaket->nama_paket ?? old('nama_paket')}}">
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <input type="hidden" name="id_paket_wisata" id="id_paket_wisata" value="{{$daftarpaket->fpaketwisata->id ??old('id_paket_wisata')}}">
                            <input type="text" class="form-control 
@error('id_paket_wisata') is-invalid @enderror" placeholder="Id Paket Wisata" id="Id Paket Wisata" name="id_paket_wisata" value="{{$daftarpaket->fpaketwisata->id_paket_wisata ?? old('nama_paket')}}" aria-label="Id Paket Wisata" aria-describedby="cari" readonly>
                            <button class="btn btn-warning" type="button" data-bs-toggle="modal" id="cari" data-bs-target="#staticBackdrop"></i>
                                Cari Id Paket Wisata</button>
                        </div>
                    </div>
                    <div class="form-group">
...
@error('harga_paket') is-invalid @enderror" id="harga_paket" placeholder="Harga Paket" name="harga_paket" value="{{$daftarpaket->harga_paket ?? old('harga_paket')}}">
    ...
    <script>
        ...
        function pilih(id, paket_wisata) {
            ...
            document.getElementById('nama_paket').value = paket_wisata
        }
    </script>
    @endpush